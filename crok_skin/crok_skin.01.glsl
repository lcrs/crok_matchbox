...
float perlinnoise( vec2 co ) {
	...
	for( int i = 0; i < octaves; i++ ) {
		...
		total += interpolatednoise( vec2( 5*i + co.x * frequency / zoom_small / overall_zoom, co.y * frequency / zoom_small / overall_zoom ) ) * amplitude;
	}
...
float voronoi( in vec2 x )
{
    ...
    for( int i=-1; i<=1; i++ )
    {
        ...
        o = 0.5 + 0.5*sin( mod(1.0 + 6.2831*o, 2*PI) ); // animate
